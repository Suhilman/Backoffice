...
toast.configure()
export default function BeetstorePage() {
    ...
    return allOutlets.map((item, index) => {
      ...
      if (item.Location) {
        ...
        return {
          ...
          status_integrate,
          activation_date_beetstore: item.activation_date_beetstore ? dayjs(item.activation_date_beetstore).format('DD MMMM YYYY') : "-",
          update_date_beetstore: item.update_date_beetstore ? dayjs(item.update_date_beetstore).format('DD MMMM YYYY') : "-"
        };
      } else {
  ...
  const columns = [
      ...
      cell: (rows) => {
        return (
          rows.status_integrate.key ? (
            <div>{rows.activation_date_beetstore}</div>
          ): "-"
        )
      }
    },
    {
      ...
      cell: (rows) => {
        return (
          rows.status_integrate.key ? (
            <div>{rows.update_date_beetstore}</div>
          ): "-"
        )
      }
    },
